  ...
  PUBLIC SECTION.
    ...
    METHODS get_pdf_64
      IMPORTING
                ...
                vaLUE(io_plant) type ZI_GATEENTRY_hdr-Plant
...
*  endSELECT.
 ...
 data(DEL_NOTE) = lv_DEL-qty_de.

  select * from   "#EC CI_ALL_FIELDS_NEEDED
  ZI_GATEENTRY_ITM
  WHERE Zgate = @io_gate
...
*|      <quantitydelnote>{ DEL_NOTE }</quantitydelnote>| &&
*|    </Subform4>| &&
*
*|    <Tablefom>| &&
*|      <Table1>| &&
...
*    |<Supp_add></Supp_add>| &&
*    |<Subform4>| &&
*    |<grosswight></grosswight>| &&
*    |<tarrewight></tarrewight>| &&
*    |<PKGWGT></PKGWGT>| &&
*    |<netwight></netwight>| &&
*    |</Subform4>| &&
*
...
|      <gateoutdate>{ gateouteate }</gateoutdate>| &&
|      <gateouttime>{ gateouttime }</gateouttime>| &&
|       <gatetype>Inward</gatetype>|  &&
|    <movemnetype>Purchase</movemnetype>| &&
|    </Subform3>| &&
